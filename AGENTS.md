...
- PR impact analysis rules live in `docs/testing/pr-impact-analysis/` and are validated by `pnpm exec tsx scripts/pr-impact-analysis.ts validate` as part of `pnpm guard`. When touching P0 UI/runtime paths, E2E coverage, or PR-validation workflows, check whether `capabilities.json`, `e2e-coverage.json`, and `owners.json` need to change together. Keep validation commands single-sourced in `e2e-coverage.json`; keep capability policy and path evidence in `capabilities.json`; keep manual QA ownership/checklists in `owners.json`.
- If PR impact validation fails, fix the rule data rather than weakening the validator. Common fixes: remove or replace stale globs that match no files, keep capability IDs in sync across all three JSON files, add coverage records for new P0 paths, update renamed E2E test paths or `ui-p0-shards.ts` shard names, and move duplicated capability validation commands out of `capabilities.json` into `e2e-coverage.json`. Analyzer recommendations are additive/advisory only; never use rule updates to reduce baseline CI coverage.
